...
1. **11:58** — bot Gemini orqali mavzu tanlaydi (kamera/video-montaj/to'y-video), internetdan
   (Google Search grounding) so'nggi ma'lumot topadi, so'ng shu ma'lumotdan qisqa "poster" matnlari
   (sarlavha, kalit so'z, subtitle, caption) yaratadi.
2. AI fon rasmi (Pollinations.ai, flux model) generatsiya qilinadi, so'ng sarlavha/subtitle/handle
   matnlari **dasturiy ravishda (Pillow)** rasm ustiga chiziladi — shu sababli matn har doim aniq,
   o'qilishi mumkin va buzilmagan bo'ladi (AI rasmga chizgan matn ko'pincha noto'g'ri chiqadi).
3. Tayyor poster + qisqa caption sizga (admin) **"✅ Qabul qilish"** / **"🔄 Qaytadan"** tugmalari
   bilan yuboriladi.
